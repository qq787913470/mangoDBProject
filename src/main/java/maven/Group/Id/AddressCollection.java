...
public class AddressCollection {
	...
	static String array_names[] = { "John", "Tim", "Brit", "Robin", "Smith",
			"Lora", "Jennifer", "Lyla", "Victor", "Adam" };

	static String array_address[][] = { { "US", "FL", " Miami" },
			{ "US", "FL", " Orlando" }, { "US", "CA", "San Diego" },
			{ "US", "FL", " Orlando" }, { "US", "FL", " Orlando" },
			{ "US", "NY", "New York" }, { "US", "NY", "Buffalo" },
			{ "US", "TX", " Houston" }, { "US", "CA", "San Diego" },
			{ "US", "TX", " Houston" } };
	...
	public static void main(String[] args) {
		...
		try {
			...
			for (int i = 0; i < array_names.length; i++) {
				...
				document.append(
						"address",
						new BasicDBObject("country", address[0]).append(
								"state", address[1]).append("city", address[2]));
			...
			BasicDBObject query = new BasicDBObject("age", new BasicDBObject(
					"$gt", 40));
			...
			/**** Update ****/
			// update documents found by query "age > 30" with udpateObj
			// "age = 20"
			...
			/**** Find and display ****/
			cursor = collection.find(query);
			System.out.println("Person with age > 40 after update --> "
					+ cursor.count());
